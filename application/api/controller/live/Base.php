...
class Base extends Controller {
    ...
    public function __construct()
    {
        ...
        if ( empty($input) || '[]' == $input ) {
            ...
            log_message('客户端请求数据:'.$input, 'log', '/data/log/live/');
    ...
    {
        return isset($this->_params[$key]) ? trim($this->_params[$key]) : ($default ? $default : null);
    }
    ...
    private function _verify() {
        ...
        log_message('客户端sign:'.$sign.' 服务端sign:'.$md5str, 'log', '/data/log/live/');
    ...
    private function _buildResponse(array &$array, array $field) {
        foreach ( $array as $key => $value ) {
            if ( is_array($value) ) {
                ...
                if ( ! in_array($key, $field) && ('count' != $key) && !preg_match('/^\d{4}\-\d{2}$/', $key) && !preg_match('/^\d{1,}$/', $key) ) {
                    unset($array[$key]);
                }
    ...
    {
        $list = [];
        $list = model('SysConfig')->getConfigInfo(false);
        config('cfg',$list);
        $this->appid = config('cfg')['LIVE_WX_MINI_APP_ID'];
        $this->appSecret = config('cfg')['LIVE_WX_MIN_APP_SECRET'];
        return true;
    }
    ...
    public function getUserInfo()
    {
        ...
        $userInfo = model('common/User')->cache(5)->where(['id' => $arr[1]])->find();
        if (empty($userInfo)) return [];
    ...
    public function getAnchorInfo()
    {
        ...
        $userInfo = model('common/LiveUser')->cache(5)->where(['id' => $arr[1]])->find();
        if (empty($userInfo)) return [];
        $this->anchorInfo = $userInfo->toArray();
    }
}
